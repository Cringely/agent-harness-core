...
| **Spec and plan before implementation** | Development work begins with a brainstorm, produces a written spec, and proceeds from a written plan. The spec and the plan exist as artifacts before implementation starts. The spec reads README, any declared goals doc, and active plans touching the same subsystem, and a contradiction goes to the operator as a decision, never resolved silently. | Where superpowers is installed, `brainstorming` and `writing-plans` produce the artifacts and `subagent-driven-development` executes the result; absent the plugin, the same two artifacts get written by hand before any implementation begins. A transcript-reading gate for this rule remains a plausible future mechanism, since parsing for one shape of missing artifact is the same technique `review-gate.ts` already uses to find unreviewed edits, but nothing wired for this exists yet; a prior attempt (`dispatch-audit.ts`) shipped untested and unused and was deleted rather than fixed. | prose + convention |
| **Work has to be able to end** | Close on the invariant a fix restores, not on the absence of findings, since adversarial review always finds something and "done" defined as no open finding is never reachable. Put a severity floor on what gets acted on. Correctness, security and anything that fails open get fixed in the round that found them. Naming, stale citations, comment wording and coverage gaps do not: they stay in the review comment on the change that surfaced them, which is already durable and linked to the code it reviewed. Run one review round per artifact. A second round happens only for a load-bearing finding, never as a matter of course. A finding becomes an issue only when it is a correctness or security gap outside the change under review, or a decision that belongs to the operator, and the operator is told each time one is filed. An agent doing the work never opens an issue of its own. A change that moves something a living doc describes, and leaves that doc untouched or records it as still accurate, is a correctness finding and gets fixed in the same round. A wording or precision defect in a doc edit made inside the change stays minor, and a doc-only finding never earns a second review round. Drift the change did not cause stays in the review comment. | `task-reviewer.md` and `adversarial-reviewer.md` already instruct reviewers to report every issue, including uncertain and low-severity ones, so a downstream filter can rank them. This row is that filter, and both agent defs point back at it by name. | JIT + convention |
