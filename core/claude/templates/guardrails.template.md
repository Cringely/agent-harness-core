...
| **Invariant promotion (note to mechanism)** | A rule that surfaces as the same class of failure a second time is the signal to promote it from a note to a mechanism (a schema, a test, a hook, or a documented convention a reviewer checks for); the original note then shrinks to a pointer instead of staying a full explanation nobody rereads. A rule that has only happened once stays a note; writing a mechanism for a single incident is usually wasted effort. What never promotes, however often it recurs: a one-time migration step, which is history rather than a constraint; a value tuned to one system, a threshold, a timeout, a path, since promoting it exports a number that is wrong everywhere else; and a bug already fixed upstream, where the promotion candidate is a version pin, if anything. The test either way: read the promoted form back and ask whether it states a constraint something could check. "Be careful with X" fails that test, and so does any rule whose real subject is a single machine. | None automated; this is the discipline that decides which tier a recurring rule graduates to. See agent-harness-core's `patterns/forcing-functions.md`, "Promoting a rule from note to mechanism," for the reasoning and worked examples in full. | prose + convention |
| **Spec and plan before implementation** | Development work begins with a brainstorm, produces a written spec, and proceeds from a written plan. The spec and the plan exist as artifacts before implementation starts. | Where superpowers is installed, `brainstorming` and `writing-plans` produce the artifacts and `subagent-driven-development` executes the result; absent the plugin, the same two artifacts get written by hand before any implementation begins. A transcript-reading gate for this rule remains a plausible future mechanism, since parsing for one shape of missing artifact is the same technique `review-gate.ts` already uses to find unreviewed edits, but nothing wired for this exists yet; a prior attempt (`dispatch-audit.ts`) shipped untested and unused and was deleted rather than fixed. | prose + convention |
| **Work has to be able to end** | Close on the invariant a fix restores, not on the absence of findings, since adversarial review always finds something and "done" defined as no open finding is never reachable. Put a severity floor on what gets acted on in the same round that found it (correctness, security, anything that fails open) and file everything else (naming, stale citations, coverage gaps) rather than fixing it now. Run one review round per artifact; a second round happens only for a load-bearing finding, never as a matter of course. Finding something files it; filing is complete in itself, and the decision to spend more effort belongs to whoever owns that effort, not to whoever found the issue. | `task-reviewer.md` and `adversarial-reviewer.md` already instruct reviewers to report every issue, including uncertain and low-severity ones, so a downstream filter can rank them; this row is that filter, and both agent defs point back at it by name. | JIT + convention |
| **Vendored prose is never linted** | A prose linter that flags text the project copied in verbatim, to cite rather than maintain, invites the fix that breaks the copy: edited to satisfy a finding, the file still reads well and no longer matches upstream, and nothing compares the two. A finding there can be a true positive and still must not be acted on, because the project does not author the text. | One skip-list entry per vendored tree in `lint-doc-prose.ts` and `pre-commit`, or one per-tree section in the project's Vale config; never one glob over every vendored tree, since a wholesale glob covers future trees nobody reviewed and zero findings reads as clean. Each entry carries a comment naming the tree and a probe that stays flagged outside it. See agent-harness-core's `patterns/vendored-prose-exemption.md`. | automate |
