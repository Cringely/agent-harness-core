...
| **Spec and plan before implementation** | Development work begins with a brainstorm, produces a written spec, and proceeds from a written plan. The spec and the plan exist as artifacts before implementation starts. | Where superpowers is installed, `brainstorming` and `writing-plans` produce the artifacts and `subagent-driven-development` executes the result; absent the plugin, the same two artifacts get written by hand before any implementation begins. `core/claude/hooks/dispatch-audit.ts` is a plausible future gate for this rule, since it already parses transcripts for one shape of missing artifact, but it is not wired for this yet and carries two known defects of its own; fixing it is separate work. | prose + convention |
