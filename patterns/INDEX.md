...
| [`test-falsifiability.md`](test-falsifiability.md) | A test can be structurally unable to fail, through a matcher blind to the defect, a fixture set that never reaches the guarded case, a measurement whose result cannot separate the claim from its opposite, an async test that fails by hanging instead of reporting, or a table of cases drawn from the constant under test, and none of the five shows up in a run as a failure; reading a suite finds some of these, and mutating it finds others along with the behaviour no test covers at all. |
