...
that a decompiler opens. To decide whether a building's `Container.dropOffset` applies from the cell
centre (`Map.CellCentre`) or from the building transform (`Map.CellBase`), the
...
two origins differ by 0.49 in y and `Map.ToCell` is `(int)(pos.y + 0.05f)`, so they land in
...
the `+ 0.05f` inside `ToCell`. One of the two was a correction that was itself presented as the
...
- **Name what produces the behaviour and check whether you can open it.** `fix-quality.md` patches the
  ...
  Reading `Container.Add` and `Container.Drop` out of the shipped assembly took about four tool calls,
  answered the question for every offset at once, and turned up a defect no screenshot could reach: the
  building carries two `Container` components and `GetPart<Container>()` returns only the first.
