...
**Determine the project slug first.** Derive it from the basename of the current working directory (e.g., `myproject` from `C:\Users\alice\Documents\projects\website\myproject`, or `home` if the CWD is the user's home directory `C:\Users\alice`). Use lowercase, replacing spaces with hyphens.

Gather the following from the current session and write to `{{OBSIDIAN_VAULT}}/Handoffs/<slug>/handoff-latest.md`, overwriting any previous file for that project. Also write a dated copy to `{{OBSIDIAN_VAULT}}/Handoffs/<slug>/handoff-YYYY-MM-DD-HHMMSS.md` for history. Create the `<slug>` subfolder if it doesn't exist (Write tool creates parent directories automatically).
...
- **Keep it under 500 words.** This file gets read into a fresh context window. Brevity is the point.
...
- **Register: compressed, except where reasoning lives.** A handoff is a brief to a later session, not a prose deliverable, so it takes the compressed row of `writing-style.md`'s surface routing rather than the full-prose contract. Status, Files Modified, and Task compress: drop articles and filler, fragments are fine, keep every path, command, identifier, and error string verbatim. Decisions Made and Constraints and Gotchas stay in normal prose, on the same split `writing-style.md` already applies to commit messages, because compression costs clarity in reasoning and the reasoning is the part compaction destroys first. Next Step stays normal prose under the auto-clarity rule: dropped conjunctions in a multi-step sequence can invert its order. Vale does not run on this file and should not; the template's own em dashes would read as defects.
...
Determine the project slug the same way as writing (basename of CWD). Read `{{OBSIDIAN_VAULT}}/Handoffs/<slug>/handoff-latest.md` and present a brief summary to the user:
