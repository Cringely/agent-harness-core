...
**Terms of art.** A specialist term earns its first use, not its eleventh. Once a word turns up in every paragraph it reads as a tic. Prefer the plain sentence where it does the same work ("delete the fix and check the test fails" beats "ablate the test"). Keep the term where it draws a distinction plain words blur and the reader already speaks it; in anything a non-expert reads, gloss it in place, in six words, the first time it appears. Current offenders: ablate, forcing function, invariant, seam, producer/consumer, net-negative. The same fatigue applies to rhetorical frames: one is style, four is a template.
...
- One specialist term or rhetorical frame recurs often enough to read as a tic.
