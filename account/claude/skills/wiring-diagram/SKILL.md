...
| `examples/example.pictorial.json` | Worked pictorial spec — copy this as your template. |
| `examples/example.harness.yml` | Worked WireViz spec — copy this as your template. |
| `examples/example.pictorial.{svg,png}`, `example.harness.{svg,png}` | Reference outputs to eyeball your result against. |

## Flow 1 — Pictorial

1. Copy `examples/example.pictorial.json`, edit it to your board (schema below).
...
1. Copy `examples/example.harness.yml`, edit `connectors` / `cables` / `connections`.
...
The pictorial is worthless if its layout does not match the board in the builder's hand. This bit the seed build three times. The spec already forces the physical facts as required inputs (`rows` top-to-bottom, `cols`, each pad's exact `silk`). Before you render the final PNG, run this gate against a **photo of the actual board** (in `hardware/images/` for the seed build):
...
```bash
python render_pictorial.py examples/example.pictorial.json /tmp/out.svg
bash svg_to_png.sh /tmp/out.svg /tmp/out.png
```

Pass = `/tmp/out.png` reproduces `examples/example.pictorial.png` essentials: 2×6 pad grid with silk, grey-X on the two RX columns, all eight callouts (two with resistor boxes), power-path strip, colour legend, six key points. For the harness, `render_harness.sh examples/example.harness.yml` reproduces `examples/example.harness.{svg,png}`.
