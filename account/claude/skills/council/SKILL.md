...
- Working directory `C:\Users\alice` → project folder `C--Users-alice`
- Working directory `E:\projects\myproject` → project folder `E--projects-myproject`

If the working directory is the user's home directory (`~`), use `{{HOME_SLUG}}` as the project folder. This mirrors the structure used by the memory system and the Obsidian sync hook.
