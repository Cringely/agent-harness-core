...
| `metadata.evidence_tier` | no | `live capture`, `offline test`, `vendored reference`, or `assumption` (vocabulary from `~/.claude/rules/no-overclaim.md`); any tier above `assumption` must name a checkable referent, such as a file:line, a commit hash, or pasted command output. An optimistic tier with nothing behind it is worse than no field, since it reads as evidence |
| `metadata.accepted_via` | no | how the note reached `accepted`: who approved it and on what |
...
MEMORY.md loads whole into every session, so it is the retrieval path, not a directory listing. A rejected note that keeps a full index entry means `status: rejected` is a label rather than a filter, and the rejected claim walks back into context on every session. So a rejected note drops out of the main list and is represented only by the compact `Rejected:` tombstone line (see Decision Notes). That line keeps its link, for audit and for a human who wants the reasoning; what makes it safe is that the claim itself is inline, so nothing has to open the body to know what was rejected.
...
Two constraints on what can raise a note's status or confidence. Several agents agreeing is one principal, not several, because it's one model: agent consensus is never a promotion path to `accepted`. Repetition, recency, and cross-linking never raise a note's confidence or status either; only new evidence does.
...
Each `rejected` note gets one compact line at the bottom of the group, and the line carries the rejected claim itself, not only the slug and link: `Rejected: [slug](file.md) — what was rejected, one clause`. A slug-keyed tombstone with no claim text collides with nothing, so a re-proposal phrased differently walks right past it; naming the claim narrows that gap to actual rewordings, it doesn't close it, and the fix for the rest isn't embeddings. `superseded` and `expired` notes leave the index entirely but stay on disk.
...
3. Contradiction scan: where two notes conflict, apply supersession (see below). Flag pairs that need user confirmation. Decision notes are compared only with other decision notes — decision-confidence and fact-confidence are different axes. Supersession is not the only outcome: two notes that disagree because they describe different scopes, hosts, or time windows both stay, recorded as `keep_both` with one clause on each note naming the scope that separates them. Forcing a winner there deletes the distinction that made both true.
4. Evidence mismatch: a note carrying `confidence: high` with `evidence_tier: assumption` gets flagged. The two fields answer different questions, and high confidence resting on an assumption is the exact shape of an overclaim. The field is added where a note's tier is contestable or where someone might act on the claim, never swept across the corpus: a tier written to satisfy a linter is worse than an absent field, because it reads as evidence. Notes with no `evidence_tier` are not flagged.
5. Staleness: notes with an old `updated` date, an expired `valid_until`, or `confidence: low` get flagged for refresh or deletion. Decision notes are handled by steps 11-12, not here. Notes with `status: rejected` are exempt from this pruning regardless of age or staleness: in the strongest system surveyed, TTL pruning deleted the tombstone the previous release had just built, silently reopening the path it was built to close.
6. Atomicity: notes over ~50 lines or covering multiple facts get flagged for splitting.
7. Hygiene: scan for leaked secrets, tokens, or real org/employer identifiers. This backstops the `Scan-MemorySecrets` hook, which fails open on a malformed payload and scans only two hardcoded roots, so a clean run there is not evidence the write was scanned.
8. Consolidation: if the same class of failure appears in multiple notes, promote it to a permanent rule in `~/.claude/rules/` (invariant promotion), shorten the memory note to a pointer, and repoint the index.
9. Sync check: confirm vault copies are present and the PostToolUse hook is registered.
10. Handoff backstop: scan the latest handoff file for durable facts not yet in memory; flag them for promotion.
11. Proposal digest: collect all `proposed` decision notes (including those past `valid_until`) into one bulk accept/reject/defer digest for the user. A proposal with an empty Evidence section is flagged as weak here. A previously-deferred, expired proposal gets `status: expired` and its index line removed.
12. Triggered revisits: decision notes whose `valid_until` has passed or whose "Revisit when" condition is verifiably met — prompt the revisit-and-Outcome flow. Best-effort for non-date conditions: Lint cannot verify event/metric prose; those outcomes get captured at supersession or when the user raises the topic.
