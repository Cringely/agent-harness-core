...
| `install/` | The installer that copies core into a project's `.claude/`, the `Export-Account.ps1` and `Install-Account.ps1` pair that carries the account layer between machines (see below) and `AccountShared.ps1`, the library both dot-source for the payload tables they have to agree on, `Restore-ClaudeProject.ps1` for moving a project and its session history to another machine, and the Pester suites beside them |
