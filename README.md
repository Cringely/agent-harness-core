...
| `core/claude/templates/` | `guardrails.template.md`, a settings fragment, a ceremony ledger template and a scratch `.gitignore`, all of which the installer applies, plus three reference templates it leaves in core: agent-def authoring, the reviewer-lockout protocol, and the untrusted-content boundary block |
| `patterns/` | The design docs behind the above, listed in [`patterns/INDEX.md`](patterns/INDEX.md) |
| `install/` | The installer that copies core into a project's `.claude/`, the `Export-Account.ps1` and `Install-Account.ps1` pair that carries the account layer between machines (see below), `Restore-ClaudeProject.ps1` for moving a project and its session history to another machine, and the Pester suites beside them |
