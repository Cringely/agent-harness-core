...
| `core/claude/templates/` | `guardrails.template.md`, a settings fragment and a scratch `.gitignore`, which the installer applies on every run, a ceremony ledger template it applies only under `-IncludeCeremonies`, plus three reference templates it leaves in core: agent-def authoring, the reviewer-lockout protocol, and the untrusted-content boundary block |
| `patterns/` | The design docs behind the above, listed in [`patterns/INDEX.md`](patterns/INDEX.md) |
| `install/` | The installer that copies core into a project's `.claude/`; the `Export-Account.ps1` and `Install-Account.ps1` pair that carries the account layer between machines (see below), with `AccountShared.ps1`, the library both dot-source for the payload tables they have to agree on; `Restore-ClaudeProject.ps1` for moving a project and its session history to another machine; and the Pester suites beside them |
