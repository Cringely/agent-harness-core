...
.DESCRIPTION
    ...
    alongside it and gitignores in the target: `.harness-manifest.local.json`.

    A manifest written before this fix still carries them embedded at top level. The next
    run of any installer command drops them from the committed file unconditionally, and
    persists them into the sidecar instead only when git can confirm the sidecar is covered
    by an ignore rule; when it cannot, the legacy values are dropped rather than migrated,
    same as a fresh scan's would be, until the ignore rule is fixed and the installer runs
    again.
...
if (Test-Path -LiteralPath $sidecarPath) {
    ...
    if ($rawSidecar.Trim()) {
        # Malformed content must not block the run the way a malformed manifest would: every
        # field here (coreRepo, stackDetected) is machine-local and regenerated by a plain
        # install's own refresh below, so the simpler fix is treating unparsable content as
        # absent rather than adding a repair path for a file the installer already knows how
...
# untracked copy, or neither, and prints the plan's warning. Returns which of the three it did
# ('written', 'removed' or 'skipped') so a caller reporting the outcome can tell an active
# deletion from doing nothing, rather than collapsing both into one falsy value. The path it
# writes or removes is the fixed literal Get-SidecarPlan built, never anything computed from
# $Sidecar's content.
...
$sidecarOutcome = Invoke-SidecarPlan -Plan $sidecarPlan -Sidecar $sidecar
# 'removed' is an active deletion of a stale, unprotected copy, not the same outcome as never
# having touched the file -- the two used to share one label ('skipped-unprotected'), which
# made a run that just deleted something read as though nothing had happened to it.
$sidecarAction = if ($sidecarOutcome -eq 'removed') { 'removed' } elseif ($sidecarOutcome -eq 'written') { 'written' } else { 'skipped-unprotected' }
