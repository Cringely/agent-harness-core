    ...
    It "lifts only the mcpServers key out of claude.json" {
        ...
        try {
            ...
            @{
                ...
                mcpServers = @{
                    acme = @{ type = 'stdio'; command = 'uvx'
                        args = @('--from', 'git+https://example.invalid/acme_mcp', 'acme-mcp')
                        env = @{} }
            ...
            $m.mcpServers.acme.command | Should -Be 'uvx'
...
Expected: no throw, and `mcp-servers.json: 3 server(s)` on stdout. The three are `1password`, `code-context` and `acme`.
...
Vale kit, the worktree gate and the acme server are all silently absent on
...
```powershell
    It "adds missing mcpServers entries and touches nothing else in claude.json" {
        ...
        try {
            ...
            @{ mcpServers = @{
                    acme = @{ type = 'stdio'; command = 'uvx'; args = @('acme-mcp'); env = @{} } } } |
...
Run: `pwsh -NoProfile -File install/Install-Account.Tests.ps1`
Expected: exit 1, `Tests Passed: 12, Failed: 4`. The first and second fail on `$j.mcpServers.acme` being `$null`, the third on no report text in the output, the fourth on the `wsl` entry never being added.
...
kept as the classifier on the printed line. The test asserts acme and the
...
'@ | Set-Content (Join-Path $ch 'hooks/Scan-MemorySecrets.ps1')
            ...
            @{ mcpServers = @{ acme = @{ type = 'stdio'; command = 'uvx'
                        args = @('acme-mcp'); env = @{} } } } |
    ...
    It "an install into an empty home leaves every hook command pointing at a real file" {
        ...
        try {
            ...
            (Get-Content (Join-Path $freshClaude 'skills/subagent-prompting/SKILL.md') -Raw) |
                Should -Not -Match '\{\{'
            (Get-Content (Join-Path $fresh '.claude.json') -Raw | ConvertFrom-Json).mcpServers.acme.command |
                Should -Be 'uvx'
        }
