...
- Tasks 1, 2, 3 and 13 edit files under `C:\Users\jcgam\.claude\`, which is the operator's live running configuration, not a repo copy. Every such task backs the file up first per `~/.claude/rules/change-management.md`: `Copy-Item -LiteralPath <file> -Destination "<file>.bak.$(Get-Date -AsUTC -Format 'yyyyMMdd-HHmmss')"`. The exporter drops `*.bak.*` from the payload, so these backups never reach the repo.
...
| Path | Change |
|---|---|
| `C:\Users\jcgam\.claude\hooks\Scan-MemorySecrets.ps1` | Bug 1. Derive both roots from `$HOME` with the incoming path's normalisation. |
| `C:\Users\jcgam\.claude\hooks\Sync-MemoryToObsidian.ps1` | Bug 2, six sites. Derive `$claudeHome`, the slug and the vault. |
| `C:\Users\jcgam\.claude\hooks\Lint-DocumentProse.ps1` | Bug 3. `Get-ValeConfigPath` built from nested `Join-Path`. |
| `C:\Users\jcgam\.claude\hooks\Guard-SkillSize.ps1` | Bug 4. `Get-SkillRoot` built from `$HOME`, `$env:LOCALAPPDATA` guarded. |
| `README.md` | Registration for both scripts (CONTRIBUTING Gate 1). |
| `C:\Users\jcgam\.claude\rules\harness-core.md` | Registration for both scripts, and the file whose `{{CORE_REPO}}` fold Task 6 tests. |
...
- Modify: `C:\Users\jcgam\.claude\hooks\Scan-MemorySecrets.ps1` (header comment at lines 14-16; the root block at lines 28-30)
...
```powershell
$f = 'C:\Users\jcgam\.claude\hooks\Scan-MemorySecrets.ps1'
...
```powershell
$f = 'C:\Users\jcgam\.claude\hooks\Sync-MemoryToObsidian.ps1'
Copy-Item -LiteralPath $f -Destination "$f.bak.$(Get-Date -AsUTC -Format 'yyyyMMdd-HHmmss')"
```
...
Expected: exit 1, `Tests Passed: 4, Failed: 4`. All four fail on a `Should -BeTrue` against a path that was never written, because `$np.StartsWith('C:\Users\jcgam\.claude\...')` is false for every sandbox path and the hook exits 0 at line 57 without copying.

- [ ] **Step 4: Derive the three roots at the top of the hook**

In `C:\Users\jcgam\.claude\hooks\Sync-MemoryToObsidian.ps1`, replace the block running from `$np = $f -replace '/', '\'` through the `$councilRoot` assignment (lines 13-17 as of this writing, including the `# --- Council transcripts ---` banner between them):

```powershell
$np = $f -replace '/', '\'
$vaultBase = 'C:\Users\jcgam\Documents\Obsidian Vault\Claude Code'
...
Inside the `docs/superpowers/{plans,specs}` branch, the line reading `if ($proj -eq '.claude') { $proj = 'C--Users-jcgam' }`:

```powershell
    if ($proj -eq '.claude') { $proj = $homeSlug }
```

The global plans/specs regex, currently `if ($np -match '^C:\\Users\\jcgam\\\.claude\\(plans|specs)\\([^\\]+)$') {`:

```powershell
if ($np -match ('^' + [regex]::Escape($claudeHome) + '\\(plans|specs)\\([^\\]+)$')) {
```

The destination two lines below it, currently `$dest = Join-Path $vaultBase 'C--Users-jcgam' $kind`:

```powershell
    $dest = Join-Path (Join-Path $vaultBase $homeSlug) $kind
```

And under `# --- Memory files ---`, currently `$memRoot = 'C:\Users\jcgam\.claude\projects\'`:
...
Restore the literal `$memRoot = 'C:\Users\jcgam\.claude\projects\'`, re-run, confirm `copies a memory write into a vault named by CLAUDE_OBSIDIAN_VAULT` goes red. Put it back. Separately restore `$proj = 'C--Users-jcgam'`, re-run, confirm `routes a global spec write to the slug of the running HOME` goes red. Two ablations, because two independent sites hold those two tests up. Put both fixes back and re-run to green.
...
```powershell
pwsh -NoProfile -Command "Get-ChildItem 'C:\Users\jcgam\Documents\Obsidian Vault\Claude Code' -Directory -ErrorAction SilentlyContinue | Where-Object { \$_.Name -like 'acct-hooks-*' -or \$_.Name -like 'C--Users-jcgam-AppData*' } | Select-Object -ExpandProperty Name"
```
...
C--Users-jcgam all named this workstation. Reading the vault from an
...
**Files:**
- Modify: `C:\Users\jcgam\.claude\hooks\Lint-DocumentProse.ps1` (line 71)
- Modify: `C:\Users\jcgam\.claude\hooks\Guard-SkillSize.ps1` (lines 33-37)
...
foreach ($n in 'Lint-DocumentProse.ps1', 'Guard-SkillSize.ps1') {
    $f = Join-Path 'C:\Users\jcgam\.claude\hooks' $n
    Copy-Item -LiteralPath $f -Destination "$f.bak.$(Get-Date -AsUTC -Format 'yyyyMMdd-HHmmss')"
}
```
...
In `C:\Users\jcgam\.claude\hooks\Lint-DocumentProse.ps1`, replace the single line reading `$config = Join-Path $HOME '.claude\tools\prose-lint\.vale.ini'` (line 71 as of this writing, immediately above `if (-not (Test-Path -LiteralPath $config)) { exit 0 }`) with:
...
In `C:\Users\jcgam\.claude\hooks\Guard-SkillSize.ps1`, replace the `$SkillRoots` array literal (lines 33-37 as of this writing, sitting between the `$UsableFraction = 0.92` assignment and the `ConvertTo-PositiveInt` comment block):
...
Run: `pwsh -NoProfile -File "C:\Users\jcgam\.claude\hooks\Guard-SkillSize.ps1" -SelfTest`
...
`settings.json` cannot travel as it stands. Every hook command carries an absolute path in one of three quoting forms: `& 'C:\Users\jcgam\.claude\hooks\Scan-MemorySecrets.ps1'` for the four `.ps1` hooks, `bun "C:\Users\jcgam\.claude\hooks\model-tier-gate.ts"` and the matching `bash "..."` form, and unquoted `node C:/Users/jcgam/AppData/Roaming/npm/node_modules/ccstatusline/dist/ccstatusline.js` in `statusLine.command` and two hook entries.
...
Obsidian vault path and the C--Users-jcgam project slug. On a receiver
...
on this box is /c/Users/jcgam/Documents, so that fix would break the skill
...
**Files:**
- Modify: `README.md`
- Modify: `C:\Users\jcgam\.claude\rules\harness-core.md`
...
```powershell
$f = 'C:\Users\jcgam\.claude\rules\harness-core.md'
Copy-Item -LiteralPath $f -Destination "$f.bak.$(Get-Date -AsUTC -Format 'yyyyMMdd-HHmmss')"
```
...
Insert this section into `C:\Users\jcgam\.claude\rules\harness-core.md`, after the `## After install, use what's there` section and before `## Findings flow (both channels)`. The `E:\projects\agent-harness-core` spelling is load-bearing: Task 14's export folds that exact literal into `{{CORE_REPO}}`.
...
Run: `pwsh -NoProfile -Command "Select-String -Path README.md, 'C:\Users\jcgam\.claude\rules\harness-core.md' -Pattern 'Export-Account|Install-Account' | Measure-Object | Select-Object -ExpandProperty Count"`
Expected: `6` or more. Both scripts named in both files, which is what Gate 1 asks for.

- [ ] **Step 7: Lint the two prose additions**

Run: `pwsh -NoProfile -Command "vale --config 'C:\Users\jcgam\.claude\tools\prose-lint\.vale.ini' --output=line README.md"`
...
foreach ($f in "rules/harness-core.md","hooks/harness-core-reminder.sh","skills/prose-lint/SKILL.md",
    ...
    "{0,-42} tokens={1} jcgam={2} Eprojects={3}" -f $f,
        @([regex]::Matches($t, "\{\{[A-Z_]+\}\}")).Count,
        @([regex]::Matches($t, "C:.Users.jcgam")).Count,
        @([regex]::Matches($t, "E:.projects.agent-harness-core")).Count
}
"settings.account.json literals: " + @([regex]::Matches((Get-Content "$p/settings.account.json" -Raw), "C:..Users..jcgam")).Count
"ssh.md untouched: " + (@([regex]::Matches((Get-Content "$p/rules/ssh.md" -Raw), "\{\{")).Count -eq 0)
'@
```

Expected: `tokens` at least 1 for every one of the six, `jcgam=0` and `Eprojects=0` on all six, `settings.account.json literals: 0`, and `ssh.md untouched: True`.

If `jcgam` or `Eprojects` is non-zero on one of the six, the fold table missed a spelling in that file. Fix `ConvertTo-TemplatedText` or the table in `install/AccountShared.ps1`, add the case to `install/Export-Account.Tests.ps1`, and re-run the export. Do not hand-edit the payload.
