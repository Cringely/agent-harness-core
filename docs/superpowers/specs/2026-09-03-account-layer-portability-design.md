...
`C:\Users\jcgam\.claude\`: eleven rules files, eight agent definitions, sixteen skills, a Vale
...
forms: `& 'C:\Users\jcgam\.claude\hooks\Scan-MemorySecrets.ps1'` for the four `.ps1` hooks, each
with `"shell": "powershell"`; `bun "C:\Users\jcgam\.claude\hooks\model-tier-gate.ts"` and the
matching `bash "..."` form for the `.ts` and `.sh` hooks; and unquoted
`node C:/Users/jcgam/AppData/Roaming/npm/node_modules/ccstatusline/dist/ccstatusline.js` in
...
claude-code bundle per its header at L140-150, so `C:\Users\jcgam` becomes `C--Users-jcgam` and
...
1. `hooks/Scan-MemorySecrets.ps1:29-33`. This is the `PreToolUse` secret scanner, and its roots
   are absolute literals, `$memRoot = 'C:\Users\jcgam\.claude\projects\'` and
   `$councilRoot = 'C:\Users\jcgam\.claude\council-transcripts\'`. Line 33 exits 0 when neither
...
2. `hooks/Sync-MemoryToObsidian.ps1`, six sites, not one. L14 hardcodes the vault, L17 the
   council root, L56 the memory root, L47 a regex anchored on `^C:\\Users\\jcgam\\\.claude\\`,
   and L39 and L49 the slug literal `'C--Users-jcgam'`. An environment variable for the vault
...
7. `skills/prose-lint/SKILL.md:14` hardcodes `C:\Users\jcgam\.claude\tools\prose-lint\.vale.ini`
   in the command the model is told to run. A `$HOME`-relative form is wrong here, because Git
   Bash `$HOME` on this box is `/c/Users/jcgam/Documents` (`change-management.md:122`), so that
...
8. `skills/handoff/SKILL.md:21,70` hardcode the vault's `Handoffs` path;
   `skills/council/SKILL.md:195-198` map `C:\Users\jcgam` to `C--Users-jcgam`;
...
`rules/ssh.md:27` names `C:\Users\jcgam\.ssh\config`, and `rules/change-management.md:122`
