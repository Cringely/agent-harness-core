...
onboarding flags. Only `acme` is portable (`uvx` plus a git URL). `1password` runs
...
environment variable, never inline. `acme` ships as-is and works anywhere. `1password` and
...
`uvx` for the `acme` server, and `jq` on a Linux box that falls back to
